...
const WatchlistCard = ({ stockData }: { stockData: Stock }) => {
  ...
  return (
    <div className="flex flex-col h-fit gap-2 items-center border rounded-lg py-3 px-4 border-white ">
      <div className="flex flex-row h-fit justify-between w-full">
        <div className="flex flex-row gap-2 items-center">
          <div className="rounded-ss-full">
            <img
              src={`https://assets.stockbit.com/logos/companies/${stockData.code}.png`}
              alt="logo"
              className="w-10 h-10"
            />
          </div>
        ...
        <div className="flex flex-col gap-2 text-xs items-center">
          <div
            className={`border ${
              stockData.health > 7.0
                ? "border-green-500 text-green-500"
                : stockData.health > 5.0
      ...
      <div
        className="w-full flex cursor-pointer hover:bg-opacity-65 justify-center text-semibold bg-green-700 rounded-md"
        onClick={() =>
          window.open("https://app.bibit.id/product/stock/ADRO", "_blank")
        }
      >
        BUY
      </div>
